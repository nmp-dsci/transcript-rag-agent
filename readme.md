...
### Voice-to-text (the composer mic)

`DEEPGRAM_API_KEY` (optional) turns on streaming speech-to-text in the chat
composer: press the mic, talk, and the question appears word-by-word as you
speak — interim words render as a live tail and firm up as Deepgram finalizes
them. The browser captures raw PCM with an AudioWorklet (identical across
Chrome, Safari, and Firefox — no MediaRecorder codec fork) and streams it to
`/ws/stt`, a FastAPI WebSocket relay that forwards audio to Deepgram's live
API and returns `{text, is_final, speech_final}` JSON. The key never reaches
the browser, and the frontend codes against the relay's message shape, so the
transcription vendor is swappable without touching UI code.

Without the key the feature is simply absent — `/api/health` reports
`stt: false` and the mic never renders. The same applies in demo mode; the
demo gate is HTTP middleware that WebSockets bypass, so `/ws/stt` re-checks
`YT_AGENT_DEMO_MODE` itself and refuses with close code 1008.
`YT_AGENT_STT_ENABLED=false` turns the feature off without deleting the key;
`YT_AGENT_STT_MODEL` (default `nova-3`) picks the Deepgram model. Sessions are
capped (110 s client-side, 120 s server-side) and the relay sends KeepAlives
during pauses, so an abandoned mic cannot bill unattended.
...
| `/api/guides/job/stream` | GET | Live stage and activity events for the guide job (SSE, seeded with the current state) |
| `/guides/…` | GET | The guide pages, `guide.md` copies, shared `guide.css` and reader bridge script, served read-only from the committed `guides/` tree |
...
### Field Guides (long-form, cited write-ups)

The **Field Guides** tab shows long-form guides written *from* the corpus by a
Claude Agent SDK pipeline: the agent reads every chunk of a confirmed set of
videos, extracts claims with chunk ids and verbatim quotes, composes a page,
and a verifier with no LLM resolves every `<cite data-video data-chunk
data-quote>` against Chroma before anything is published. Readers comment on
the page in the app; a revision applies the open comments as one tracked
batch and answers each with a receipt.

**The agent contract.** Every stage is a fresh SDK session (nothing resumed;
all state is a committed file, so a killed run restarts at its last completed
stage and any revision is reproducible from the repo). The agent gets
Read/Write/Edit/Glob/Grep confined to the guide directory by a `PreToolUse`
hook, plus one in-process MCP tool, `retrieve_chunks`, over the same hybrid
...
```text
guides/
  guide.css                 # the shared editorial stylesheet every guide links
  guide-bridge.js           # the reader bridge (theme, scroll, selection) — postMessage only
  index.json                # derived catalog; regenerated by every guides command
  <slug>/
    manifest.json           # title, topic, video ids, sources, models, cite counts, gaps, provenance
    guide.html              # the current page (rendered in a sandboxed iframe)
    guide.md                # the agent-facing copy: same sections, [VIDEO_ID@N] cites
    claims.json             # every cite, resolved: video ok / chunk ok / quote ok
    evidence/cluster-N.json # extraction passes: claims with chunk ids + verbatim quotes, verified
    comments.jsonl          # reader commentary (append-only; the last line per id wins)
    receipts/vN.json        # the reviser's per-comment outcome for each revision
    versions/vN.html        # immutable history; guide.html is a copy of the latest
    runs/*.jsonl            # every stage and tool event of every run
    corpus/                 # exported chunks, one .md per video — gitignored, rebuilt from Chroma
```

Ship Like a Studio was hand-run before the pipeline existed, so it is
*imported* rather than regenerated: its stylesheet became `guide.css`, every
section/heading/paragraph/list item got a stable document-order `id`
(comments anchor to ids), and each YouTube link became a video-level cite.
LLM-as-a-Judge and Production GenAI Systems were written by the pipeline.
...
uv run python -m src.cli guides markdown ship-like-a-studio                 # backfill guide.md from the page
uv run python -m src.cli guides comment llm-as-a-judge "Add the pairwise vs pointwise tradeoff" --anchor method-li4
uv run python -m src.cli guides comment llm-as-a-judge --from-json annotations.json   # e.g. Lavish annotations
