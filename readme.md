...
| `/guides/…` | GET | The guide pages, shared `guide.css` and reader bridge script, served read-only from the committed `guides/` tree |
...
The **Field Guides** tab shows long-form guides written *from* the corpus: each
one reads every chunk of a chosen set of videos and compiles a page whose
claims carry `<cite data-video="…" data-chunk="…">` markup, verified against
Chroma by `src/guides/verify.py` with no LLM in the loop. A guide is a
committed directory:
...
guides/
  ...
  <slug>/
    manifest.json           # title, topic, video ids, sources, model, cite counts, provenance
...
*imported* rather than regenerated: the importer moves its stylesheet to
`guide.css`, gives every section, heading, paragraph and list item a stable
document-order `id` (comments anchor to ids), and turns each YouTube link into
a video-level cite.

```bash
uv run python -m src.cli guides import .lavish/s20_ship-like-a-studio.html --slug ship-like-a-studio --compiled-at 2026-08-31
uv run python -m src.cli guides list                       # slugs, versions, cite rates
uv run python -m src.cli guides verify ship-like-a-studio  # re-resolve every cite; exit 1 on any failure
```

The tab stays visible in demo mode: the catalog, pages and claims are plain
GETs over committed files. The page renders in an iframe sandboxed without
`allow-same-origin`, so an agent-written page can never reach the workbench's
origin; the workbench pushes the theme in and reads section lists and text
selections out through `guides/guide-bridge.js`.
